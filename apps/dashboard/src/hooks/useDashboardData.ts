...
export const useDashboardData = (token: string, onSessionExpired: () => void, source: string | null = null) => {
  ...
  // Sync source into ref and reset pagination when it changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (!token) return
    fetchData({ token, page, source: sourceRef.current }).then((result) => {
